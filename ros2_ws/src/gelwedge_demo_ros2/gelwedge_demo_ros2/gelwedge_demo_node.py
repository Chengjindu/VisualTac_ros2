...
class GelWedgeDemoNode(Node):
    def __init__(self) -> None:
        ...
        self.matcher = find_marker.Matching(
            ...
            theta=setting.theta,  # new optional, or keep default
            dmin_ratio=setting.dmin_ratio,
            dmax_ratio=setting.dmax_ratio,
            moving_max_ratio=setting.moving_max_ratio,
            flow_diff_ratio=setting.flow_diff_ratio,
            cost_ratio=setting.cost_ratio,  # main cost scaling
            K1=setting.K1,
            K2=setting.K2,
            reference_dx_=setting.reference_dx  # ✅ your new parameter
        )
        ...
        self.timer = self.create_timer(0.00, self._loop_once)
    ...
    def _loop_once(self) -> None:
        ...
        # Perspective transform (if matrix available)
        if self.M is not None:
            try:
                frame = cv2.warpPerspective(frame, self.M, (self.width, self.height))
            except Exception as e:
                self.get_logger().warn(f"warpPerspective failed: {e}")

            # # One-pass warp to final processed size/orientation
        # if self.M_comp is not None:
        #     try:
        #         frame_proc = cv2.warpPerspective(frame, self.M, self.out_size)
        #     except Exception as e:
        ...
        # Marker tracking
        # m_centers = utilities_demo.marker_center(frame_proc, debug=False)
        ...
        try:
            wrench = utilities_demo.compute_resultant_flow_wrench_int(
                flow,
                pixel_size_m=self.pixel_size_m,
                k_force_per_m=self.k_force_per_m,
                origin_px=(self.width / 2.0, self.height / 2.0),
